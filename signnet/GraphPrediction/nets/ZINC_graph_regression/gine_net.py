...
class GINENet(nn.Module):
    def __init__(self, net_params):
        ...
        if self.pe_init == 'rand_walk' or self.lap_lspe:
            ...
            # NoPE or LapPE
            self.layers = nn.ModuleList([ GINEConv(MLP(hidden_dim, hidden_dim, hidden_dim, 2, use_bn=self.batch_norm, dropout=dropout, activation='relu')) for _ in range(self.n_layers-1) ]) 
            self.layers.append(GINEConv(MLP(hidden_dim, hidden_dim, out_dim, 2, use_bn=self.batch_norm, dropout=dropout, activation='relu')))
