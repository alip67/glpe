...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    if net_params['pe_init'] == 'lap_pe':
        ...
        dataset._add_p_positional_encodings(net_params['pos_enc_dim'],net_params['p_lap_epochs'],net_params['p_laplacian'],device)
        print("[!] Time taken: ", time.time()-tt)
    ...
    train_loader = DataLoader(trainset, num_workers=0, batch_size=params['batch_size'], shuffle=True, collate_fn=dataset.collate)
    val_loader = DataLoader(valset, num_workers=0, batch_size=params['batch_size'], shuffle=False, collate_fn=dataset.collate)
    test_loader = DataLoader(testset, num_workers=0, batch_size=params['batch_size'], shuffle=False, collate_fn=dataset.collate)
...
def main():    
    ...
    parser.add_argument('--config', default='configs/gin/GIN_ZINC_LapPE.json' ,help="Please give a config.json file with training/model/data/param details")
    ...
    parser.add_argument('--lap_lspe', help="Please give a value for lap lspe")
    ...
    if args.lap_lspe is not None:
        print('LAP LSPE ARG', args.lap_lspe)
        net_params['lap_lspe'] = args.lap_lspe == 'True'
        if args.lap_lspe: assert args.pe_init == 'lap_pe'
